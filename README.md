...
- **Player Tab**: Choose a portrait and set your character's name, age, height, gender, pronouns, personality, backstory, current appearance, ancestry, and class/role when editing a scenario. Each playthrough starts with its own copy. During play, **Player** opens character information in the journal and inventory panel. Age and height can be updated after changes or transformations; height accepts your preferred units. Personality guides the voice, motivations, and choices in suggested responses and the Write button, and can be edited as your character develops. It uses the existing generation calls. **Scenario Settings**, alongside Player and Inventory in the composer toolbar, holds feature toggles and time/survival pacing.
...
- **Optional Player Stats**: Enable six ability scores (Strength, Dexterity, Constitution, Intelligence, Wisdom, and Charisma), level, HP, and armor class. During play, ability and survival values appear in a separate **Stats** tab when either system is enabled. Values are read-only until you explicitly select **Override stats**. The override lets you type level, ability scores, HP, armor, and survival values directly, including values beyond the usual caps or RPG point budget. Custom values persist after saving and reloading; later attribute spending adds to your custom scores. Saving, leaving the tab, or reloading locks editing again. Player overrides do not alter NPC budgets or the scenario’s shared level cap. Scenario creation still lets you set initial values. Turning a system off keeps its saved values.
- **Optional RPG Progression**: Enable **RPG progression** in scenario creation or live **Scenario Settings**. It is off by default. Progression uses player stats; dice remain a separate opt-in. Each attribute starts at **8**, with **12 starting attribute points** and **2 points per level above level 1** by default. Configure the starting level, starting pool, points per level, and an optional maximum level; a blank maximum is unlimited. The starting level applies when a run starts or progression is first enabled. Spend points with **+** in the scenario editor or live **Stats** tab; **Override stats** allows direct value editing without spending or refunding earned points. Successful real rolls earn **DC × 10 XP** and failed rolls earn **5 XP**. Skips earn none. Each level costs **100 × current level XP**, and level-ups grant the configured points. XP is awarded once per roll, remains recorded across narration retries, and rewinds with a removed roll. These calculations make no AI calls.
...
- **Automatic Character Discovery**: Named people introduced in completed story responses are added to the roster automatically. The normal story response uses `[NPC_MET: name|occupation|physical description]` for a person you meet and `[NPC_MENTION: name|occupation]` for someone only talked about. Mentions create an entry without a physical description. On meeting that person later, local matching updates the same entry and the AI supplies a concise physical description in that same response, preserving existing stats and edits. Existing characters with missing descriptions can be filled on their next meeting. No separate description request is made. Legacy `[NPC: name|occupation]` flags remain supported. Only valid flags create roster entries. Names must contain the person’s name alone; dialogue fragments such as “I’m Lira” and unnamed roles are rejected. Plain story text does not create additional characters. Existing names, titles, and unique aliases are matched locally to avoid duplicates. Matching and stat/personality generation run locally; descriptions add a small amount of text to the normal story response without another AI call. Older visible story messages are also checked when you reopen a playthrough.
...
- **NPC Personality and Persuasion**: NPC personalities, temperaments, motivations, and mannerisms remain stable when you learn more about their occupation. Edit them in the character sheet; authored personalities are retained when creating a sheet. The narrator and private conversations receive these saved traits. With dice enabled, personality influences social difficulty, including romance, without automatically blocking an attempt. A sufficiently high charisma result can overcome reluctance. The AI is instructed to honor the recorded result and explain the reaction in character without rewriting the NPC's whole personality. Without dice, personality guides the story naturally. These instructions use existing calls.
...
- **CYOA (Choose Your Own Adventure)**: Generate several response options shaped by your player's personality and the current scene. Click an option to put its text in the message box, then review or edit it and send when ready. Selecting an option does not send a message, trigger a roll, advance time, or make an AI call.
...
- **Character Equipment**: Add starting equipment in the scenario's Player tab, one item per line, with quantities such as `2 × Torches`. New playthroughs begin with those items in Inventory. During play, the Player sheet and Inventory share the same items and quantities. Editing a sheet preserves unrelated inventory changes and item details; lost or consumed items do not return on reload. Inventory remains accessible with journal tracking turned off. NPC character sheets also have age, height, and an equipment editor. Their gear is saved separately by owner, using the same item and quantity handling. Current measurements and equipment are included in the existing narration prompts without another AI call. Leave unknown measurements blank; they are not guessed from occupation.
